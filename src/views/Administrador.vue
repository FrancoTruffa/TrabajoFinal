    ...
    <v-layout row wrap>
      <v-flex v-if="obtenerLista" xs12 pt-5>
        ...
        <v-card v-for="(denuncias, index) in obtenerLista" :key="index">
          <v-card-text>
          {{denuncias.nombre_cliente}} DENUNCIO A <a :href="'#/VerTrabajador/' + denuncias.id_trabajador_denunciado" style="color:#FFFFFF;">{{ denuncias.apellido_trabajador_denunciado }} {{denuncias.nombre_trabajador_denunciado}}</a>
          <v-btn 
           color="red" 
           dark
           small
        ...
        <v-expansion-panel>
          ...
          >
            <template v-slot:header>
              ...
              <div v-if="mes.nombre == 'Agosto'">{{ mes.nombre }} -  Cantidad: 0</div>
              <div v-if="mes.nombre == 'Septiembre'">{{ mes.nombre }} -  Cantidad: 0</div>
              <div v-if="mes.nombre == 'Octubre'">{{ mes.nombre }} -  Cantidad: 1</div>
              <div v-if="mes.nombre == 'Noviembre'">{{ mes.nombre }} -  Cantidad: 0</div>
              <div v-if="mes.nombre == 'Diciembre'">{{ mes.nombre }} -  Cantidad: 10</div>
            </template>
            <v-card v-for="(pagos, index) in obtenerPagos" :key="index">
              <v-card-text v-if="pagos.fecha.substring(5,7) === String(mes.id)">Nro Comprobante: {{pagos.comprobante}} - Año: {{pagos.fecha.substring(0,4)}}</v-card-text>
            </v-card>
          </v-expansion-panel-content> 
